...
std::string GaussFFT::Quote() {
  return "Arc, amplitude, and curvature sustain a similar relation to each other as time, motion, and velocity, or as "
...
std::vector<size_t> GaussFFT::FindGridSizeAfterPadding(NRLib::Variogram* variogram,
                                                       ...
                                                       size_t            nz,
                                                       double            dz) {
  std::vector<size_t> out  = NRLib::FindNDimPadding(*variogram, nx, dx, ny, dy, nz, dz);
...
/******************************************************************/
py::array_t<double>
GaussFFT::Simulate(NRLib::Variogram* variogram, size_t nx, double dx, size_t ny, double dy, size_t nz, double dz) {
...
py::array_t<double> GaussFFT::SimulateWithAdvancedSettings(NRLib::Variogram* variogram,
  ...
  if (ny <= 1U || dy < 0.0) {
    ...
    result = GaussFFT::Simulate3D(variogram, nx, dx, ny, dy, nz, dz, padding_x, padding_y, padding_z, scaling_x,
                                  scaling_y, scaling_z);
  }
...
std::vector<double> GaussFFT::Simulate1D(NRLib::Variogram* variogram,
  ...
  values.resize(nx);
  NRLib::Simulate1DGaussianField(*variogram, nx, dx, values,
...
std::vector<double> GaussFFT::Simulate2D(NRLib::Variogram* variogram,
  ...
  NRLib::Simulate2DGaussianField(*variogram, nx, dx, ny, dy, 1, fields,
                                 NULL,  // Will use NRLib::Random state
                                 padding_x, padding_y, scaling_x, scaling_y);
...
std::vector<double> GaussFFT::Simulate3D(NRLib::Variogram* variogram,
  ...
  NRLib::Simulate3DGaussianField(*variogram, nx, dx, ny, dy, nz, dz, 1, fields, padding_x, padding_y, padding_z,
