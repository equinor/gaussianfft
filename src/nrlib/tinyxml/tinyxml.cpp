...
const TiXmlNode* TiXmlNode::FirstChild(const char* _value, int& index) const {
  const TiXmlNode* node;
  index = 0;
  for (node = firstChild; node; node = node->next) {
...
const TiXmlElement* TiXmlNode::FirstChildElement(const char* _value, int& index) const {
  const TiXmlNode* node;

  index = 0;

  for (node = FirstChild(_value, index); node; node = node->NextSibling(_value)) {
    if (node->ToElement())
      return node->ToElement();
  }
  return 0;
...
bool TiXmlDocument::LoadFile(FILE* file, TiXmlEncoding encoding) {
  ...
  // The document parser may decide the document ends sooner than the entire
  // file, however.
  TIXML_STRING data;
  data.reserve(length);
  ...
  while (*p) {
    ...
    if (*p == 0xa) {
      ...
      if (*(p + 1) == 0xa) {
        ...
        // it was followed by something else...that is presumably characters
        // again.
        ++p;
        lastPos = p;
        assert(p <= (buf + length));
...
void TiXmlAttribute::SetDoubleValue(double _value) {
  char buf[256];
#if defined(TIXML_SNPRINTF)
  TIXML_SNPRINTF(buf, sizeof(buf), "%f",
                 _value);  // NBNB-PAL: ISO C++ does not support the ‘%lf’ printf format
#else
