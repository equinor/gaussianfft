...
std::vector<double> Interpolation::Interpolate1D(const std::vector<double>& x_in,
  ...
  if (method == "spline") {
    ...
    throw Exception(
        "Interpolation method 'linear' does not take extrapolation value as "
        "input.\n");
  } else {
    throw Exception("Interpolation method '" + method + "' is not implemented.\n" +
                    "Valid methods are: linear, spline\n");
...
std::vector<double> Interpolation::Spline1D(const std::vector<double>& x_in,
  ...
  for (size_t i = 0; i < n_out; ++i) {
    ...
    bool   outside_defined_interval = false;  // Defined interval for the spline functions is from x_in[0] to
                                              // x_in[n_in].

    // Unless specified an extrapolation value, we use the first spline or last
    // spline for values outside the defined intervals.
    if (idx == -1) {
      idx = 0;
      if (x < x_in[0]) {
