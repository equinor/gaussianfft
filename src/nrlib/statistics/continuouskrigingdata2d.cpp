...
ContinuousKrigingData2D::ContinuousKrigingData2D(const std::vector<Point*>&    data_points,
  ...
  // map over values from std::vector<Point*> to vectors or coordinates and data
  // (as indices and data in IndexKrigingData2D).
...
//---------------------------------------------------------------------
void ContinuousKrigingData2D::AddData(double x, double y, double value) {
...
//---------------------------------------------------------------------
void ContinuousKrigingData2D::WriteToFile(const std::string& fileName) const {
  std::ofstream file;
  ...
  for (unsigned int k = 0; k < data_.size(); k++)
    file << std::fixed << std::setprecision(2) << std::setw(5) << xvector_[k] << " " << std::setw(5) << yvector_[k]
         << " " << std::setw(10) << data_[k] << std::endl;
  file.close();
}

//----------------------------------------------------------------------
size_t ContinuousKrigingData2D::CountDataInBlock(size_t imin, size_t imax, size_t jmin, size_t jmax) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void ContinuousKrigingData2D::AddDataToBlock(KrigingData2D& kriging_data_block,
                                             size_t         imin,
                                             size_t         imax,
                                             size_t         jmin,
                                             size_t         jmax) const
