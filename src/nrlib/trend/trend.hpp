...
class Trend {
 public:
  ...
  virtual double GetValue2D(double                     s1,
                            double                     s2,
...
class TrendConstant : public Trend {
 public:
  ...
  virtual double GetValue(double /*s1*/, double /*s2*/, double /*s3*/, const NRLib::Volume& /*volume*/) const {
    return trend_;
  }
...
class Trend1D : public Trend {
 public:
  ...
  virtual std::vector<int>    GetTrendSize(void) const;
  virtual int                 GetTrendDimension(void) const;
  virtual double              GetValue(double s1, double s2, double s3) const;
  virtual double              GetValue(double s1, double s2, double s3, const NRLib::Volume& volume) const;
 ...
 private:
  ...
  int                 reference_;  // 1 means s1 direction, 2 means s2 direction, 3 means s3
                                   // direction
  double              inv_s1_inc_;
};
...
class Trend2D : public Trend {
 ...
 private:
  int                   reference1_;  // 1 means s1 direction, 2 means s2 direction, 3 means s3
                                      // direction
  int                   reference2_;  // 1 means s1 direction, 2 means s2 direction, 3 means s3
                                      // direction
