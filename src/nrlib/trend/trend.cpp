...
double Trend::GetValue2D(double                     s1,
                         double                     s2,
                         const std::vector<double>& trend,
                         size_t                     ns1_in,
                         size_t                     ns2_in,
                         size_t                     start_ind) const {
  // Bilinear interpolation method - always assume 2D plane (can be used by 2D
  // trend and 3D trend)
  ...
  if (i0 < 0) {
    ...
    val =
        delta_s1 * trend[start_ind + i1 + (ns2 - 1) * ns1] + (1 - delta_s1) * trend[start_ind + i0 + (ns2 - 1) * ns1];
...
TrendConstant::TrendConstant(double trend) : trend_(trend) {}

TrendConstant::TrendConstant(const TrendConstant& trend) : Trend(), trend_(trend.trend_) {}

TrendConstant::TrendConstant() {}

TrendConstant::~TrendConstant() {}

void TrendConstant::Truncate(double min, double max) {
  if (trend_ < min)
    trend_ = min;
  if (trend_ > max)
    trend_ = max;
...
std::vector<double> TrendConstant::GetIncrement(void) const {
...
std::vector<int> TrendConstant::GetTrendSize(void) const {
...
int TrendConstant::GetTrendDimension(void) const {
  int dim = 0;
  return (dim);
}

NRLib::Point TrendConstant::DrawPoint(const NRLib::Volume& volume) const {
...
bool TrendConstant::CheckForNegative() const {
  if (trend_ < 0)
    return (false);
  else
    return (true);
...
Trend1D::Trend1D(const std::vector<double>& trend, int reference) : trend_(trend) {
...
Trend1D::Trend1D(const std::vector<double>& trend, int reference, double dz) : trend_(trend) {
...
Trend1D::Trend1D(const Trend1D& trend)
    : Trend(), trend_(trend.trend_), reference_(trend.reference_), inv_s1_inc_(trend.inv_s1_inc_) {}
...
Trend1D::Trend1D(const std::string& filename, int reference) {
  ...
  if (reference > 3 || reference < 1) {
    errTxt =
        "Wrong reference value in Trend1D. Reference must be either 1, 2 or 3. "
        "Value used is " +
        NRLib::ToString(reference) + "\n";
    throw NRLib::Exception(errTxt);
  }
  reference_ = reference;
}
...
void Trend1D::AddConstant(double c) {
  for (size_t i = 0; i < trend_.size(); i++)
    trend_[i] += c;
}

void Trend1D::Truncate(double min, double max) {
  for (size_t i = 0; i < trend_.size(); i++) {
    if (trend_[i] < min)
      trend_[i] = min;
    if (trend_[i] > max)
...
double Trend1D::GetTrendElement(int i, int j, int k) const {
  double value = 0;
  if (reference_ == 1)
    value = trend_[i];
...
double Trend1D::GetValue(double s1, double s2, double s3) const {
...
double Trend1D::GetValue(double s1, double s2, double s3, const NRLib::Volume& volume) const {
  // Linear interpolation method
  ...
  double val = GetValue(s1rel, s2rel,
                        s3rel);  // GetValue checks for values outside volume
  return val;
}

std::vector<double> Trend1D::GetIncrement(void) const {
  std::vector<double> increment(3);
  for (int i = 0; i < static_cast<int>(increment.size()); i++)
    increment[i] = 0;
...
std::vector<int> Trend1D::GetTrendSize(void) const {
  std::vector<int> size(3);
  for (int i = 0; i < 3; i++)
    size[i] = 0;
...
int Trend1D::GetTrendDimension(void) const {
  int dim = 1;
  return (dim);
}

double Trend1D::GetMaxValue(void) const {
...
double Trend1D::GetMinValue(void) const {
  assert(trend_.size() > 0);
  double min = trend_[0];
  for (size_t i = 1; i < trend_.size(); i++)
    if (trend_[i] < min)
...
double Trend1D::GetMeanValue(void) const {
  assert(trend_.size() > 0);
  double mean = 0.0;
  for (size_t i = 1; i < trend_.size(); i++)
    mean += trend_[i];
...
void Trend1D::ScaleToAverage(double target) {
...
bool Trend1D::CheckForNegative() const {
  bool ok = true;
  for (size_t i = 1; i < trend_.size(); i++)
    if (trend_[i] < 0)
      ok = false;
...
//-------------------------------------------------------------------//
Trend2D::Trend2D(const NRLib::Grid2D<double>& trend, int reference1, int reference2) : trend_(trend) {
...
void Trend2D::AddConstant(double c) {
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
      trend_(i, j) += c;
}

void Trend2D::Truncate(double min, double max) {
  for (size_t i = 0; i < trend_.GetNI(); i++) {
    for (size_t j = 0; j < trend_.GetNJ(); j++) {
      if (trend_(i, j) < min)
        trend_(i, j) = min;
...
double Trend2D::GetTrendElement(int i, int j, int k) const {
  int ii = 0;
  int jj = 0;
  if (reference1_ == 1)
    ii = i;
...
double Trend2D::GetValue(double s1, double s2, double s3, const NRLib::Volume& volume) const {
  // Linear interpolation method
  double s1rel, s2rel;
  double val;
...
double Trend2D::GetValue(double s1, double s2, double /*s3*/) const {
...
std::vector<double> Trend2D::GetIncrement(void) const {
  std::vector<double> increment(3);
...
std::vector<int> Trend2D::GetTrendSize(void) const {
  std::vector<int> size(3);
  if (reference1_ == 1 && reference2_ == 2) {  // constant in z
    size[0] = ns1_;
    size[1] = ns2_;
...
int Trend2D::GetTrendDimension(void) const {
  int dim = 2;
  return (dim);
}

double Trend2D::GetMaxValue(void) const {
...
double Trend2D::GetMinValue(void) const {
  assert(trend_.GetN() > 0);
  double min = trend_(0);
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
double Trend2D::GetMeanValue(void) const {
  assert(trend_.GetN() > 0);
  double mean = 0.0;
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
void Trend2D::ScaleToAverage(double target) {
  double avg_trend = GetMeanValue();
  double scale     = target / avg_trend;
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
bool Trend2D::CheckForNegative() const {
  bool ok = true;
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
      if (trend_(i, j) < 0)
...
Trend3D::Trend3D(NRLib::Grid<double>& values) : Trend() {
  trend_ = values;
}
...
Trend3D::Trend3D(const std::string& file_name, bool binary, Endianess file_format) : Trend() {
  ...
  if (binary == false)
    ...
    ReadTrend3DBinary(file_name, file_format, err_txt, trend_);
...
void Trend3D::AddConstant(double c) {
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
void Trend3D::Truncate(double min, double max) {
  for (size_t i = 0; i < trend_.GetNI(); i++) {
    for (size_t j = 0; j < trend_.GetNJ(); j++) {
      for (size_t k = 0; k < trend_.GetNK(); k++) {
        if (trend_(i, j, k) < min)
...
double Trend3D::GetValue(double s1, double s2, double s3) const {
  assert(s1 >= 0 && s2 >= 0 && s3 >= 0);
  assert(s1 <= 1 && s2 <= 1 && s3 <= 1);

  // Linear interpolation method between two bilinear interpolations (above,
  // below)
...
double Trend3D::GetValue(double s1, double s2, double s3, const NRLib::Volume& volume) const {
  // Linear interpolation method
  ...
  if (s1rel < 0)  // This check is made because GetValue does not accept
                  // location points outside volume
    s1rel = 0.0;
  if (s2rel < 0)
    s2rel = 0.0;
  if (s3rel < 0)
  ...
  if (s1rel > 1)  // This check is made because GetValue does not accept
                  // location points outside volume
    s1rel = 1.0;
  if (s2rel > 1)
    s2rel = 1.0;
...
std::vector<double> Trend3D::GetIncrement(void) const {
...
std::vector<int> Trend3D::GetTrendSize(void) const {
  std::vector<int> size(3);
...
int Trend3D::GetTrendDimension(void) const {
  int dim = 3;
  return (dim);
}

double Trend3D::GetMaxValue(void) const {
  assert(trend_.GetN() > 0);
  double max = trend_(0);
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
double Trend3D::GetMinValue(void) const {
  assert(trend_.GetN() > 0);
  double min = trend_(0);
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
double Trend3D::GetMeanValue(void) const {
  assert(trend_.GetN() > 0);
  double mean = 0.0;
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
...
void Trend3D::ScaleToAverage(double target) {
  // normalize trend to have expectation = factor.
  double avg_trend = GetMeanValue();
  double scale     = target / avg_trend;
...
bool Trend3D::CheckForNegative() const {
  bool ok = true;
  for (size_t i = 0; i < trend_.GetNI(); i++)
    for (size_t j = 0; j < trend_.GetNJ(); j++)
      for (size_t k = 0; k < trend_.GetNK(); k++)
