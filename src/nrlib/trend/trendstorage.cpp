...
TrendConstantStorage::TrendConstantStorage(const double& value, const bool& estimate)
    : mean_value_(value), estimate_(estimate) {}
...
TrendConstantStorage::TrendConstantStorage(const TrendConstantStorage& trend_storage)
    : mean_value_(trend_storage.mean_value_), estimate_(trend_storage.estimate_) {}

TrendConstantStorage::~TrendConstantStorage() {}

Trend* TrendConstantStorage::GenerateTrend(const std::string& /*path*/,
    ...
    if (type == TrendStorage::MEAN) {
      ...
      if (typeid(*mean_trend) == typeid(NRLib::TrendConstant)) {
        ...
        if (z_n > 0) {
          ...
          var = RMISSING;
          tmp_err_txt +=
              "Error: Unable to estimate variance as the wells only contain "
              "missing values.\n";
        }
        ...
        std::vector<double> x;
        std::vector<double> z;
        PreprocessData1D(s, blocked_logs, trend_cube_sampling[mean_reference - 1], x, z);
        ...
        std::vector<std::vector<double> > trend_cube_trend_2D(trend_cube_n1,
                                                              std::vector<double>(trend_cube_n2, RMISSING));
        int                               k_dummy = 0;
        ...
        PreprocessData2D(s1, s2, blocked_logs, trend_cube_sampling, x, y, z);

        int z_n = 0;
        for (size_t i = 0; i < z.size(); i++) {
          double z_mean_i =
              Interpolate(trend_cube_sampling[0], trend_cube_sampling[1], trend_cube_trend_2D, x[i], y[i]);
...
Trend1DStorage::Trend1DStorage(const std::string& file_name,
                               ...
                               const bool&        estimate)
    : file_name_(file_name), reference_parameter_(reference_parameter), estimate_(estimate) {}
...
Trend* Trend1DStorage::GenerateTrend(const std::string&                       path,
                                     const std::vector<std::string>&          trend_cube_parameters,
                                     const std::vector<std::vector<double> >& trend_cube_sampling,
                                     const std::vector<std::vector<double> >& blocked_logs,
                                     const std::vector<std::vector<double> >& s1,
  ...
  if (estimate_ == true) {
    ...
    if (type == TrendStorage::MEAN) {
      ...
      if (reference == 1 || reference == 2) {
        PreprocessData1D(s, blocked_logs, trend_cube_sampling[reference - 1], x, y);
        ...
        if (CheckConfigurations1D(x, y, trend_cube_sampling[reference - 1], bandwidth, errTxt)) {
          EstimateTrend1D(x, y, trend_cube_sampling[reference - 1], trend_estimate, bandwidth, errTxt);
        }
      }
      if (errTxt == "") {
      ...
      if (typeid(*mean_trend) == typeid(NRLib::TrendConstant) || typeid(*mean_trend) == typeid(NRLib::Trend1D)) {
        ...
        if (reference == 1 || reference == 2) {
          ...
          if (CheckConfigurations1D(x, y, trend_cube_sampling[0], bandwidth, errTxt)) {
            EstimateVariance1D(x, y, trend_cube_sampling[reference - 1], trend_cube_trend, variance_estimate,
                               bandwidth, errTxt);
          }
        }
      } else if (typeid(*mean_trend) == typeid(NRLib::Trend2D)) {
        ...
        if (reference == 1) {
          ...
          if (CheckConfigurations1D(x, z, trend_cube_sampling[reference - 1], bandwidth, errTxt)) {
            EstimateVariance1D(x, z, trend_cube_sampling[reference - 1], trend_cube_trend_dummy, variance_estimate,
                               bandwidth, errTxt);
          }
        } else if (reference == 2) {
          ...
          if (CheckConfigurations1D(y, z, trend_cube_sampling[reference - 1], bandwidth, errTxt)) {
            EstimateVariance1D(y, z, trend_cube_sampling[reference - 1], trend_cube_trend_dummy, variance_estimate,
                               bandwidth, errTxt);
          }
        }
      }
    ...
    if (file_format < 0) {
      ...
      if (trend_cube_sampling[reference - 1][0] < s_min) {
        ...
        errTxt += "The maximum value of the 1D trend in " + file_name_ +
                  " \n"
                  " is higher than the maximum value of " +
                  trend_cube_parameters[reference - 1] + "\n";
      } else {
    ...
    if (errTxt == "") {
      ResampleTrend1D(trend_sampling, trend_values, trend_cube_sampling[reference - 1], resampled_trend);
...
Trend* Trend2DStorage::GenerateTrend(const std::string&                       path,
                                     const std::vector<std::string>&          trend_cube_parameters,
                                     const std::vector<std::vector<double> >& trend_cube_sampling,
                                     const std::vector<std::vector<double> >& blocked_logs,
                                     const std::vector<std::vector<double> >& s1,
  ...
  if (estimate_ == true) {
    ...
    if (type == TrendStorage::MEAN) {
      ...
      bool valid_dataset = CheckConfigurations2D(x, y, z, trend_cube_sampling[0], trend_cube_sampling[1], bandwidth_x,
                                                 bandwidth_y, errTxt);

      if (valid_dataset) {
        EstimateTrend2D(x, y, z, trend_cube_sampling[0], trend_cube_sampling[1], trend_surface, bandwidth_x,
                        bandwidth_y, errTxt);
      }
    ...
    if (type == TrendStorage::VAR) {
      if (mean_trend != NULL) {
        /* -- obtain trend surface -- */
        std::vector<std::vector<double> > trend_cube_trend(trend_cube_n1,
                                                           std::vector<double>(trend_cube_n2, RMISSING));
        int                               k_dummy = 0;
        for (int i = 0; i < static_cast<int>(trend_cube_n1); ++i) {
          for (int j = 0; j < static_cast<int>(trend_cube_n2); ++j) {
        ...
        std::vector<std::vector<double> > trend_cube_variance(trend_cube_n1,
                                                              std::vector<double>(trend_cube_n1, RMISSING));

        bandwidth_x        = CalculateBandwidth(x, scale, 0.167);
        bandwidth_y        = CalculateBandwidth(y, scale, 0.167);

        bool valid_dataset = CheckConfigurations2D(x, y, z, trend_cube_sampling[0], trend_cube_sampling[1],
                                                   bandwidth_x, bandwidth_y, errTxt);
        if (valid_dataset) {
          EstimateVariance2D(x, y, z, trend_cube_sampling[0], trend_cube_sampling[1], trend_cube_trend,
                             trend_cube_variance, bandwidth_x, bandwidth_y, errTxt);
        }
