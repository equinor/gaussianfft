...
void EstimateConstantTrend(const std::vector<std::vector<double> >& blocked_logs, double& trend) {
...
//-------------------------------------------------------------------------------
void PreprocessData0D(const std::vector<std::vector<double> >& blocked_logs, std::vector<double>& y) {
  y.clear();
...
void WriteTrend1D(const std::string& filename, const std::vector<double>& s, const std::vector<double>& trend) {
  std::ofstream file;
  OpenWrite(file, filename);

  file << std::fixed << std::setprecision(6) << s.front() << "\n" << s[1] - s[0] << "\n" << s.back() << "\n";
...
void ReadTrend1DPlainAscii(const std::string& file_name, std::string& /*errText*/, std::vector<double>& trend1d) {
  std::ifstream file;
  OpenRead(file, file_name);
  std::string dummyStr;
  int         line = 0;
...
int GetTrend1DFileFormat(const std::string& file_name, std::string& errText) {
  std::string   dummyStr;
  std::ifstream file;
...
void ResampleTrend1D(const std::vector<double>& x,
  ...
  while (i < nx - 1) {
    while (j < ny && y[j] >= x[i] && y[j] < x[i + 1]) {
      InterpolateTrend1DValues(x[i], x[i + 1], fx[i], fx[i + 1], y[j], fy[j]);
      j++;
    }
...
void EstimateTrend1D(const std::vector<double>& x,
  ...
  // if the number of sample points is too large, we regrid to save computation
  // time.
  if (x0.size() > 128) {
  ...
  // if the sample size is large (> 128^2 = 16384), linear binning is used to
  // save computation time.
  if (x.size() > 16384) {
    MakeBinnedDataset1D(x, y, weights, x0_regridded, y_binned, w_binned);

    x_binned = x0_regridded;
  }

  // finds the effective sample size (in ''the best of all worlds'') and use
  // this to make an adaptive bandwidth for the Kernel estimator.
  ...
  // the effective weighted sample size has to be less than the actual weighted
  // sample size
  if (w_binned_size < effective_sample_size) {
    effective_sample_size = w_binned_size;
    LogKit::LogFormatted(LogKit::High,
                         "\nWARNING : The effective sample size used in estimating 1D trend is "
                         "larger than the actual sample size.");
    LogKit::LogFormatted(LogKit::High,
                         "\n          The local linear model is reduced to a "
  ...
  while (!complete_line) {
    LocalLinearRegression1D(x_binned, y_binned, w_binned, bandwidth, effective_sample_size, x0_regridded, y0_regridded,
                            complete_line);
    ...
    if (nr_iterations > 1e4) {
      ...
      errTxt +=
          "       The interpolated region is too large compared to the support "
          "of the data.\n";
      errTxt +=
          "       Reduce the size of the estimated region or estimate a "
          "constant value.\n";
      break;
    }
  }
...
void PreprocessData1D(const std::vector<std::vector<double> >& s,
  ...
  // Note that s1 and s2 are corrected/centered according to
  // trend_cube_sampling, we have to add x_min or y_min to obtain the original
  // s1 and s2 scale.
  ...
  if (!CheckIfVectorIsSorted(x0)) {
    ...
    if (x0_min < x_min - x_upp && x_max + x_upp < x0_max) {
      ...
      if (x0_min < x_min - x_large || x_max + x_large < x0_max) {
        LogKit::LogFormatted(LogKit::High,
                             "\nWARNING : The defined region for 1D trend/variance is large "
                             "compared to the support of the data.");
        LogKit::LogFormatted(LogKit::High,
                             "\n          This can result in unstable estimates. Consider using "
                             "an alternative (low-dimensional) method.\n");
      }
        ...
        if (n_samples < 10) {
          ...
          errTxt +=
              "       The algorithm requires at least 11 observations to "
              "run.\n";
          valid_dataset = false;
        } else {
          if (bandwidth < 1e-5) {
            errTxt += "Error: Unable to estimate 1D trend/variance.\n";
            errTxt +=
                "       The spread in the data is too low to provide a valid "
                "bandwidth.\n ";
            valid_dataset = false;
          } else {
            if (n_samples < 32) {
              LogKit::LogFormatted(LogKit::High,
                                   "\nWARNING : The sample size is relatively "
                                   "small for 1D trend/variance estiamtion.");
              LogKit::LogFormatted(LogKit::High,
                                   "\n          This can result in unstable estimates. Consider "
                                   "using an alternative (low-dimensional) method.\n");
            }
          }
        }
      }
...
void MakeBinnedDataset1D(const std::vector<double>& x,
  ...
  // We also bin the weights if they are given as input, else we return the
  // local weights
  bool   w_input            = w != NULL;
...
void EstimateVariance1D(const std::vector<double>& x,
  ...
  if (y_var == RMISSING) {
    ...
    if (x.size() > 16384) {
      MakeBinnedDataset1D(x, y_y_mean_squared, weights, x0_regridded, y_y_mean_squared_binned, w_binned);
      x_binned = x0_regridded;
    }
    ...
    if (w_binned_size < y_var_weight) {
      ...
      LogKit::LogFormatted(LogKit::High,
                           "\nWARNING : The effective sample size used in estimating 1D "
                           "variance is larger than the actual sample size.");
      LogKit::LogFormatted(LogKit::High,
                           "\n          The local linear model is reduced to a "
                           "standard linear model.");
    }
    ...
    KernelSmoother1DLine(x_binned, y_y_mean_squared_binned, w_binned, bandwidth, 0.0, x0_regridded, variance_regridded,
                         w0_regridded, complete_line_dummy);
...
void EstimateTrend2D(const std::vector<double>&         x,
  ...
  std::vector<std::vector<double> > z0_regridded(x0_regridded.size(),
                                                 std::vector<double>(y0_regridded.size(), RMISSING));
  std::vector<std::vector<double> > w0_regridded_dummy(x0_regridded.size(),
                                                       std::vector<double>(y0_regridded.size(), RMISSING));
  ...
  if (z_binned_size < effective_sample_size) {
    ...
    LogKit::LogFormatted(LogKit::High,
                         "\nWARNING : The effective sample size used in estimating 2D trend is "
                         "larger than the actual sample size.");
  ...
  while (l < 1e4) {
    LocalLinearRegression2DSurface(x_binned, y_binned, z_binned, w_binned, bandwidth_x, bandwidth_y,
                                   effective_sample_size, x0_regridded, y0_regridded, z0_regridded, w0_regridded_dummy,
                                   complete_surface);
  ...
  if (l > 1e4) {
    ...
    errTxt +=
        ...
        "of the data.\n";
    errTxt +=
        "       Reduce the size of the estiamted region or use a "
...
void PreprocessData2D(const std::vector<std::vector<double> >& s1,
  ...
  // Note that s1 and s2 are corrected/centered according to
  // trend_cube_sampling, we have to add x_min and y_min to obtain the original
  // s1 and s2 scale
...
bool CheckConfigurations2D(const std::vector<double>& x,
  ...
  if (!CheckIfVectorIsSorted(x0) || !CheckIfVectorIsSorted(y0)) {
    ...
    if (x0.front() < x_min - x_upp && x_max + x_upp < x0.back() && y0.front() < y_min - y_upp &&
        y_max + y_upp < y0.back()) {
      errTxt += "Error: Unable to estimate 2D trend/variance.\n";
      errTxt +=
          "       The interpolated region is too large compared to the support "
          "of the data.\n";
      errTxt +=
          "       Reduce the size of the estiamted region or use a low "
          "dimensional method.\n";
      valid_dataset = false;
    } else {
      if (x0.front() < x_min - x_large || x0.back() > x_max + x_large || y0.front() < y_min - y_large ||
          y0.back() > y_max + y_large) {
        LogKit::LogFormatted(LogKit::High,
                             "\nWARNING : The defined region for 2D trend/variance is large "
                             "compared to the support of the data.");
        LogKit::LogFormatted(LogKit::High,
                             "\n          This can result in unstable estimates. Consider using "
                             "an alternative (low-dimensional) method.\n");
        ...
        if (n_samples < 10) {
          errTxt += "Error: Unable to estimate 2D trend/variance.\n";
          errTxt +=
              "       Too few observations. The algorithm requires at least 11 "
              "observations to run.\n";
          valid_dataset = false;
        } else {
          if (bandwidth_x < 1e-5 || bandwidth_y < 1e-5) {
            ...
            if (n_samples < 48) {
              LogKit::LogFormatted(LogKit::High,
                                   "\nWARNING : The sample size is relatively "
                                   "small for 2D trend/variance estiamtion.");
              LogKit::LogFormatted(LogKit::High,
                                   "\n          This can result in unstable estimates. Consider "
                                   "using an alternative (low-dimensional) method.\n");
            }
...
//-------------------------------------------------------------------------------
double CalculateBandwidth(const std::vector<double>& x, const double& scale, const double& power) {
...
void BilinearBinning(const std::vector<double>&         x,
  ...
  for (size_t i = 0; i < z.size(); i++) {
    if ((x0_first - delta < x[i]) && (x[i] < x0_last + delta) && (y0_first - delta < y[i]) &&
        (y[i] < y0_last + delta)) {
      size_t k;
      size_t l;
      double w_y;
...
//--------------------------------------------------------------------------------------------------------------
size_t FindLowerBoundInSortedVector(const std::vector<double>& x, const double& x_0) {
  size_t n   = x.size();
  size_t low = 0;
  size_t upp = n - 1;
...
//--------------------------------------------------------------------------------------------------------------
size_t FindUpperBoundInSortedVector(const std::vector<double>& x, const double& x_0) {
  size_t n   = x.size();
  size_t low = 0;
  size_t upp = n - 1;
...
void EstimateVariance2D(const std::vector<double>&               x,
  ...
  if (z_var == RMISSING) {
    ...
    std::vector<std::vector<double> > z0_weighted_var_regridded(x0_regridded_n,
                                                                std::vector<double>(y0_regridded_n, RMISSING));
    ...
    if (x_binned.size() < effective_sample_size) {
      ...
      LogKit::LogFormatted(LogKit::High,
                           "\nWARNING : The effective sample size used in estimating 2D "
                           "variance is larger than the actual sample size.");
      LogKit::LogFormatted(LogKit::High,
                           "\n          The local linear is reduced to a "
                           "standard linear model.");
    }
    ...
    KernelSmoother2DSurface(x_binned, y_binned, z_z_mean_squared_binned, w_binned, bandwidth_x, bandwidth_y, 0.0,
                            x0_regridded, y0_regridded, z0_var_regridded, w0_regridded, complete_surface_dummy);
...
void ReadTrend2DPlainAscii(const std::string& file_name, std::string& err_txt, NRLib::Grid2D<double>& trend2d) {
  std::ifstream file;
  OpenRead(file, file_name);
  int line = 0;
