...
int qrf(GeMatrix<MX>& A, DenseVector<VX>& c, DenseVector<VY>& d, bool& singular) {
...
template <typename MX, typename VX, typename VY>
void rsolv(GeMatrix<MX>& A, DenseVector<VX>& d, DenseVector<VY>& b) {
  typedef typename MX::ElementType T;
...
void qrsolv(GeMatrix<MX>& A, DenseVector<VX>& c, DenseVector<VY>& d, DenseVector<VZ>& b) {
  typedef typename MX::ElementType T;
  int                              n = A.numRows();
  for (int j = 0; j < n - 1; ++j) {
    T   sum;
...
template <typename T>
void qrinv(GeMatrix<FullStorage<T, ColMajor> >& A, GeMatrix<FullStorage<T, ColMajor> >& InvA) {
  bool                   singular;
...
void svdGMP(const GeMatrix<FullStorage<T, Order> >& A,
