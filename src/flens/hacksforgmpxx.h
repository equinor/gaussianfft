...
struct Initializer<Array<T> > {
  static void initialize(Array<T>& a) { std::uninitialized_fill_n(a.data(), a.length(), T()); }
};

#  ifdef GMP
template <typename T, typename U>
struct Initializer<Array<__gmp_expr<T, U> > > {
  static void initialize(Array<__gmp_expr<T, U> >& a) {
...
struct Initializer<FullStorage<T, Order> > {
  static void initialize(FullStorage<T, Order>& fs) {
...
struct Initializer<FullStorage<__gmp_expr<T, U>, Order> > {
  static void initialize(FullStorage<__gmp_expr<T, U>, Order>& fs) {
    std::uninitialized_fill_n(fs.data(), fs.numRows() * fs.numCols(), __gmp_expr<T, U>(0));
  }
};
#  endif
