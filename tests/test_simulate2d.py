...
class TestSimulate2D(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dy = 1.5
        grf.seed(12321)
        cls.field2d = grf.simulate(variogram, cls.nx, cls.dx, cls.ny, cls.dy).reshape((cls.nx, cls.ny), order='F')

    def test_shape(self):
        self.assertTupleEqual(self.field2d.shape, (self.nx, self.ny))

    def test_gradient(self):
        diffs0 = np.diff(self.field2d, axis=0)
        diffs1 = np.diff(self.field2d, axis=1)
    ...
    def test_rolled_gradient(self):
        ...
        # was insufficient
        diffs0 = np.diff(np.roll(self.field2d, int(self.nx/2), axis=0), axis=0)
        diffs1 = np.diff(np.roll(self.field2d, int(self.ny/2), axis=1), axis=1)
        self.assertLess(
