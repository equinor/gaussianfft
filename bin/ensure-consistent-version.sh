...
num_source_distributions="$(find "$ARTIFACT_DIR" -name "gaussianfft-*.tar.gz" | wc -l)"
readonly num_source_distributions
...
expected_num_artifacts="$(find "$ARTIFACT_DIR" -name "gaussianfft-${version}-*.whl" -type f | wc -l)"
readonly expected_num_artifacts

actual_num_artifacts="$(find "$ARTIFACT_DIR" -name "gaussianfft-*.whl" -type f | wc -l)"
readonly actual_num_artifacts
...
actual_total_num_artifacts=$(find "$ARTIFACT_DIR" -mindepth 1 | wc -l)
readonly actual_total_num_artifacts
