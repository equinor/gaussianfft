...
STATE_MACHINE: StateMachineType = {
    "states": {"fetching", "fetched", "preparing", "prepared", "done", "failed", None},
    "transitions": {
        None: {"fetching", "failed"},
        "fetching": {"fetched", "failed"},
        "fetched": {"preparing", "failed"},
...
class GatherArmPerformanceLibraries:
    ...
    def prepare(self):
        if sys.version_info < (3, 14):
            raise RuntimeError("Python 3.14 or higher is required to prepare ARM Performance Libraries")
